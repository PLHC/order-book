...
orderExecution OrderBook::checkExecution(Order* orderToBeChecked){
    auto volume = static_cast<int32_t>(orderToBeChecked->getterVolume());
    auto price = orderToBeChecked->getterPrice();
    auto& LinkedList = orderToBeChecked->getterOrderDirection()==buy? offers_ : bids_;
    auto nextOrder = LinkedList.getterHead();

    while(nextOrder && volume > 0 &&
          (orderToBeChecked->getterOrderDirection() == buy ?
           nextOrder->getterPrice() <= price : nextOrder->getterPrice() >= price)){
        volume -= static_cast<int32_t>(nextOrder->getterVolume());
        nextOrder = nextOrder->getterNextBO();
    }
    if(volume<=0) return fullExecution;
    else if(volume<orderToBeChecked->getterVolume()) return partialExecution;
...
void OrderBook::insertion(Order* newOrder){
    ...
    auto price = newOrder->getterPrice();
    while(nextBO &&
          (newOrder->getterOrderDirection()==buy? price<=nextBO->getterPrice() : price>=nextBO->getterPrice())){
...
void OrderBook::performExecution(Order* executingOrder) {
    ...
    Order* nextOrder;
    auto volume = executingOrder->getterVolume();
    auto price = executingOrder->getterPrice();

    while(orderToBeUpdated && volume > 0 && (executingOrder->getterOrderDirection() == buy ?
                         orderToBeUpdated->getterPrice() <= price : orderToBeUpdated->getterPrice() >= price)){
        nextOrder = orderToBeUpdated->getterNextBO();
        if(volume >= orderToBeUpdated->getterVolume()) {
            volume -= orderToBeUpdated->getterVolume();
            orderToBeUpdated->updateVolume(0);
            //orderToBeUpdated: record changes in memory
            deletion(orderToBeUpdated);
        }
        else{
            orderToBeUpdated->updateVolume(orderToBeUpdated->getterVolume() - volume);
            volume=0;
    ...
    if(volume==0){
        ...
        executingOrder->updateVolume(volume);
        //update database for executing order
    }
}
